...
import { supabase } from "@/integrations/supabase/client";
import { User } from "@supabase/supabase-js";
import { Brain, LogOut, TrendingUp, Target, Calendar, PlayCircle, FileText, TrendingDown, Clock, List } from "lucide-react"; 
import { useToast } from "@/hooks/use-toast";
// 💡 Import Dialog components
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";


// Define a type for a quiz, NOW INCLUDING necessary details
type Quiz = {
    id: string;
    title: string;
    description: string;
    time_limit: string | null; // e.g., "00:30:00"
...
const UserDashboard = () => {
    ...
    const fetchDashboardData = async (userId: string) => {
        try {
            // 1. Fetch Available Quizzes (Active only)
            // 💡 MODIFIED SELECT: Fetch time_limit and sections for the modal
            const { data: quizzesData, error: quizzesError } = await supabase
                .from('quizzes')
            ...
            if (!quizzesError) {
                // Ensure sections is treated as an array for the Quiz type
                const normalizedQuizzes = quizzesData.map(quiz => ({
                    ...quiz,
                    sections: quiz.sections || [],
                }))
                setAvailableQuizzes(normalizedQuizzes as Quiz[]);
            }

            // 2. Fetch User Stats (Placeholder logic using quiz_results)
            const { data: resultsData, error: resultsError } = await supabase
                .from('quiz_results')
                .select('score, total_questions, completed_at')
                .eq('user_id', userId)
    ...
    const handleStartConfirmation = (quiz: Quiz) => {
        setSelectedQuiz(quiz);
    };

    // 💡 Function to navigate after confirmation
    const handleConfirmStart = () => {
        if (selectedQuiz) {
            navigate(`/quiz/${selectedQuiz.id}`);
            setSelectedQuiz(null); // Close modal and clear state
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-background to-muted">
            {/* Header (Renamed title to match suggestion) */}
            <header className="border-b border-border bg-card/50 backdrop-blur-sm">
                <div className="container mx-auto flex items-center justify-between px-4 py-4">
                    <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-primary to-secondary">
                        ...
                        <div className="space-y-4">
                            {availableQuizzes.map(quiz => (
                                <div key={quiz.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                                    ...
                                    {/* 💡 MODIFIED BUTTON: Triggers the confirmation modal */}
                                    <Button onClick={() => handleStartConfirmation(quiz)}>
                                        <PlayCircle className="mr-2 h-4 w-4" />
                                        Start Quiz
                                    </Button>
                                </div>
                ...
                <Dialog open={!!selectedQuiz} onOpenChange={() => setSelectedQuiz(null)}>
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Are you ready to begin this assessment? Once started, the timer will begin.
                            </DialogDescription>
                        </DialogHeader>
                        
                        <div className="space-y-4 py-4">
                            <p className="text-base text-foreground">{selectedQuiz.description}</p>

                            <div className="flex items-center space-x-3 text-sm font-medium">
                                <Clock className="h-4 w-4 text-primary" />
                                <span>Time Limit: 
                                    <span className="font-semibold ml-1">
                                        {selectedQuiz.time_limit || 'None'}
                                    </span>
                                </span>
                            </div>
                            ...
                            <div className="space-y-1">
                                ...
                                <ul className="list-disc list-inside ml-4 text-sm text-muted-foreground">
                                    {selectedQuiz.sections.map((section, index) => (
