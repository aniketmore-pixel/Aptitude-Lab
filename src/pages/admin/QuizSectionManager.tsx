...
type Question = {
    ...
    quiz_section_name: string; // This will hold the TRIMMED string now
...
const QuizSectionManager = () => {
    ...
    // --- Data Fetching (FIX APPLIED TO DATA MAPPING) ---
    const fetchQuizData = useCallback(async () => {
        ...
        try {
            ...
            const rawData = quizData as RawQuizData;
            
            // Safely parse JSONB sections and TRIM whitespace from each section name
            const sectionsArray: string[] = (Array.isArray(rawData.sections) ? rawData.sections : [])
                .map(s => String(s).trim()); 
            ...
            if (sectionsArray.length > 0 && openSection === null) {
                 setOpenSection(sectionsArray[0]);
            }
            ...
            // 💡 FIX 3: MAP THE FETCHED DATA to TRIM the quiz_section_name BEFORE grouping.
            const normalizedQuestions: Question[] = (questionsData as any[]).map(q => ({
                ...q,
                // Ensure the section name is trimmed before being stored in state
                quiz_section_name: String(q.quiz_section_name).trim(), 
            }));
    ...
    const handleAddQuestion = (sectionName: string) => {
        // NOTE: We should ideally navigate back to the Quiz Editor and re-fetch to ensure consistency
    ...
    // --- Rendering Logic (FINAL GROUPING FIX) ---
    if (loading) {
    ...
    const questionsBySection = questions.reduce((acc, q) => {
        // Since we trimmed q.quiz_section_name on fetch, we can use it directly in the check.
        const sectionName = sections.includes(q.quiz_section_name) ? q.quiz_section_name : 'Unassigned Questions';
        
        // Use the confirmed sectionName for grouping
                                                ...
                                                <div className="flex-grow pr-4">
                                                    ...
                                                    {(q.topics && q.topics.length > 0) && ( 
                                                         <span className="text-xs text-muted-foreground flex items-center">
                                                             <Tag className="h-3 w-3 mr-1" /> Topic: {q.topics[0].name}
                                                         </span>
                                                    )}
                                                </div>
